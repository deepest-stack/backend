CREATE OR REPLACE FUNCTION ${graph_name}.gen_rand_num(
    IN seed BIGINT
) RETURNS INTEGER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION ${graph_name}.modularity(
    IN norm REAL
) RETURNS REAL AS $$
DECLARE
    q REAL;
...
CREATE OR REPLACE FUNCTION ${graph_name}.convert2canonical(
...
CREATE OR REPLACE FUNCTION ${graph_name}.send_msg(
...
BEGIN
    ...
    UNION
    SELECT louvain_edges_dual.dst_id AS vid, v_send_msg.cid FROM
    v_receive_msg JOIN louvain_edges_dual ON louvain_edges_dual.dst_id = v_receive_msg.vid
    JOIN v_send_msg ON v_send_msg.vid = louvain_edges_dual.src_id) AS t1
    DISTRIBUTED BY (vid);
...
CREATE OR REPLACE FUNCTION ${graph_name}.num_edges_v2c(
) RETURNS VOID AS $$
BEGIN
    ...
    UNION
    SELECT message_tmp.vid, message_tmp.cid, weight FROM
    message_tmp JOIN louvain_edges_dual ON message_tmp.vid = louvain_edges_dual.dst_id
    JOIN vertex_state ON louvain_edges_dual.src_id = vertex_state.vid AND message_tmp.cid = vertex_state.cid) AS t1
...
CREATE OR REPLACE FUNCTION ${graph_name}.select_opt_community(
    IN tot_weight REAL
) RETURNS VOID AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION ${graph_name}.update_community_info(
) RETURNS VOID AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION ${graph_name}.update_vertex_state(
) RETURNS VOID AS $$
BEGIN
    ...
    UPDATE vertex_state SET rand_num = ${graph_name}.gen_rand_num(rand_num::BIGINT);
END;
$$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION ${graph_name}.louvain_stage1(
...
DECLARE
    ...
    new_modularity REAL;
BEGIN
    SELECT ${graph_name}.modularity(tot_weight) INTO current_modularity;
    WHILE current_iter_stage1 <= max_iter_stage1 LOOP
        -- send message
        PERFORM ${graph_name}.send_msg(current_iter_stage1);
        -- attach num_edges to message
        PERFORM ${graph_name}.num_edges_v2c();
        -- select community
        PERFORM ${graph_name}.select_opt_community(tot_weight);
        ...
        PERFORM ${graph_name}.update_community_info();
        -- check modularity change
        SELECT ${graph_name}.modularity(tot_weight) INTO new_modularity;
        ...
        END IF;
        -- update vertex state
        PERFORM ${graph_name}.update_vertex_state();
        DROP TABLE opt_community_tmp;
...
CREATE OR REPLACE FUNCTION ${graph_name}.update_edges(
) RETURNS VOID AS $$
BEGIN
    ...
    DROP TABLE louvain_edges, louvain_edges_dual, vertex_state;

    ALTER TABLE new_louvain_edges RENAME TO louvain_edges;

    PERFORM ${graph_name}.convert2canonical('louvain_edges');

    CREATE TEMP TABLE louvain_edges_dual AS
...
CREATE OR REPLACE FUNCTION ${graph_name}.louvain_stage2(
) RETURNS VOID AS $$
BEGIN
    PERFORM ${graph_name}.update_edges();
...
CREATE OR REPLACE FUNCTION ${graph_name}.init_louvain(
) RETURNS VOID AS $$
BEGIN
    -- init vertex state
    CREATE TEMP TABLE vertex_state AS
    SELECT 
        ...
        vid AS cid, 
        0 AS community_updated
    FROM ${graph_name}.vertex_all DISTRIBUTED BY (vid);
    ...
    SELECT 
        vid AS cid, 
        0 AS num_edges, 
        in_degree + out_degree AS tot_degree
    FROM ${graph_name}.vertex_all DISTRIBUTED BY (cid);

    -- init edges
    CREATE TEMP TABLE louvain_edges AS
    SELECT src_id, dst_id, 1 AS weight
    FROM ${graph_name}.edge_all DISTRIBUTED BY (src_id);
...
CREATE OR REPLACE FUNCTION ${graph_name}.init_weighted_louvain(
) RETURNS VOID AS $$
BEGIN
    -- init edges
    CREATE TEMP TABLE louvain_edges AS
    ...
    FULL JOIN
    (SELECT dst_id, sum(weight) AS deg FROM louvain_edges_dual GROUP BY dst_id) AS t2
...
BEGIN
    ...
    'CREATE TEMP TABLE louvain_edges AS
    SELECT src_id, dst_id, 1 AS weight
    FROM ${graph_name}.edge_'||edge_type||' DISTRIBUTED BY (src_id)';
...
CREATE OR REPLACE FUNCTION ${graph_name}.louvain(
    IN max_iter INTEGER,
    IN max_iter_stage1 INTEGER,
    IN max_iter_no_change_stage1 INTEGER,
    IN weighted BOOLEAN,
...
BEGIN
    IF weighted THEN
        PERFORM ${graph_name}.init_weighted_louvain();
    ELSE
        PERFORM ${graph_name}.init_louvain();
    END IF;
    ...
    WHILE current_iter <= max_iter LOOP
        ...
        PERFORM ${graph_name}.louvain_stage1(max_iter_stage1, max_iter_no_change_stage1, tot_weight);
        IF current_iter = 1 THEN
            CREATE TEMP TABLE vertex_cid AS
            SELECT vid, cid FROM vertex_state
            DISTRIBUTED BY (cid);
        ELSE
            CREATE TEMP TABLE new_vertex_cid AS
            SELECT vertex_cid.vid, vertex_state.cid FROM
            vertex_cid JOIN vertex_state ON vertex_cid.cid = vertex_state.vid
            DISTRIBUTED BY (cid);
            DROP TABLE vertex_cid;
            ALTER TABLE new_vertex_cid RENAME TO vertex_cid;
        END IF;

        PERFORM ${graph_name}.louvain_stage2();

        current_iter := current_iter + 1;

    END LOOP;

    DROP TABLE IF EXISTS community_info, louvain_edges, louvain_edges_dual, vertex_state;
...
BEGIN
    IF weighted THEN
        PERFORM ${graph_name}.init_weighted_louvain(edge_type);
    ELSE
        PERFORM ${graph_name}.init_louvain(edge_type);
    END IF;

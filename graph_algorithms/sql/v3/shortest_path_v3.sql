...
BEGIN
    ...
    VALUES (ARRAY[source], ARRAY[0], source, ARRAY[0.0], 0.0, 0);
...
BEGIN
    ...
    PERFORM public.parse_path(
        result_table_name, 
        'SELECT pathid, UNNEST(vertex_idx), UNNEST(path) 
        FROM paths WHERE vid = ''' || target || ''''
        );
    RETURN result_count;
...
BEGIN
    ...
    ELSE
        PERFORM public.parse_path_with_attr(
            result_table_name, 
            weight_column_type,
            'SELECT pathid, UNNEST(vertex_idx), UNNEST(path), UNNEST(edges_attr) 
            FROM paths WHERE vid = ''' || target || ''' AND weight = ' || min_weight
        );
    END IF;

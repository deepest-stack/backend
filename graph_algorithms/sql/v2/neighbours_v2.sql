...
BEGIN
    ...
    IF result_type = 0 THEN
        EXECUTE 'CREATE TABLE ' || result_table_name || '(vid INTEGER, attr INTEGER) DISTRIBUTED BY (vid)';
    ...
    ELSE
        EXECUTE 'CREATE TABLE ' || result_table_name || '(src_id INTEGER, dst_id INTEGER) DISTRIBUTED BY (dst_id)';
